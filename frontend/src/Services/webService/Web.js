...
export async function UpdateBroker(data, token) {
    try {
        ...
        if (data.brokerid == 1 || data.brokerid == 2 || data.brokerid == 6) {
...
export async function BrokerResponsedataByClient(data, token) {
    try {
        const res = await axios.post(`${Config.base_url}api/client/orderlistdetail`, data, {
            headers: {
                data: {},
                'Authorization': `${token}`,
            },
...
export async function PlaceOrderApi(data, token, brokerstatus) {
    try {
        ...
        if (brokerstatus == 1) {
            ...
            url = `${Config.base_url}aliceblue/placeorder`;
...
export async function ExitPlaceOrderData(data, token, brokerstatus) {
    try {
        ...
        if (brokerstatus == 1) {
            ...
            url = `${Config.base_url}aliceblue/exitplaceorder`;
        } else if (brokerstatus == 3) {
...
export async function GetNotificationData(data, token) {
    try {
        const res = await axios.get(
            `${Config.base_url}api/list/notificationwithplan/${data.user_id}?page=${data.page}`,
            {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
...
export async function GetLivePricedataByID(id, token) {

    try {
        const res = await axios.get(`${Config.base_url}api/getliveprices/${id}`, {
            headers: {
                Authorization: `${token}`,
            },
        });
...
export async function PlaceOrderofmultiplesignal(data, token, brokerstatus) {
    try {
        ...
        if (brokerstatus == 1) {
            ...
            url = `${Config.base_url}aliceblue/multipleplaceorder`;
        } else if (brokerstatus == 3) {
...
export async function ExitPlaceOrderofmultiplesignal(data, token, brokerstatus) {
    try {
        ...
        if (brokerstatus == 1) {
            ...
            url = `${Config.base_url}aliceblue/multipleexitplaceorder`;
        } else if (brokerstatus == 3) {
...
export async function BrokerOrderlistByClient(data, token) {
    try {
        let url;
        if (data.brokerid == 1) {
            url = "angel/checkorder";
        } else if (data.brokerid == 2) {
            url = "aliceblue/checkorder";
        } else if (data.brokerid == 3) {
            url = "kotakneo/checkorder";
        } else if (data.brokerid == 4) {
            url = "markethub/checkorder";
        } else if (data.brokerid == 5) {
            url = "zerodha/checkorder";
        } else if (data.brokerid == 6) {
            url = "upstox/checkorder";
        } else if (data.brokerid == 7) {
            url = "dhan/checkorder";
        }
